...
const r1cs = path.join(build, "merkle.r1cs");
...
const zkey0 = path.join(build, "merkle_0000.zkey");
const zkey1 = path.join(build, "merkle_final.zkey");
const vkey = path.join(build, "merkle_verification_key.json");
...
run(
  `npx --no-install snarkjs zkey contribute "${zkey0}" "${zkey1}" --name=local -v -e="demo merkle zkey"`
);
